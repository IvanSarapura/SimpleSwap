...
contract SimpleSwap is ERC20, Ownable {
    ...
    /// @dev Nested mapping: token0 => token1 => TokenPairReserves
    ...
    event Swap(address indexed tokenIn, address indexed tokenOut, uint amountIn, uint amountOut, address indexed to);
    ...
    function addLiquidity(
        ...
        liquidity = calculateLPTokensToMint(amountA, amountB, reserveA, reserveB);
        _mint(to, liquidity);
    ...
    /**
     * @notice Updates the stored reserves for a token pair
     * @dev Ensures reserves are stored in correct order based on token sorting
     * @param tokenA Address of the first token
     * @param tokenB Address of the second token
